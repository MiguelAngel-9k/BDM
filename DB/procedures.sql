    ...
    CASE
        ...
        WHEN _OP = 'ELI' THEN #ELIMINAR ELEMENTO DE LA LISTA
            DELETE FROM D_LISTAS_DETALLE 
                WHERE ID_LISTA = _LISTA AND ID_OBJETO = _OBJ;
...
BEGIN
    ...
    INSERT INTO VENTAS(
        ...
        _USUARIO, 
        SYSDATE(), 
        (SELECT SUM(PRECIO) FROM CARRITO_COMPRAS_DETALLE WHERE ID_CARRITO = @CARRITO)
    );

    SET @TICKET = (SELECT ID_VENTA FROM VENTAS ORDER BY ID_VENTA DESC LIMIT 1);

    WHILE(
            SELECT 
                ID_CARRITO 
            FROM CARRITO_COMPRAS_DETALLE 
            WHERE ID_CARRITO = @CARRITO LIMIT 1
        ) > 0 DO

        SET @OBJ = (SELECT ID_OBJETO FROM CARRITO_COMPRAS_DETALLE WHERE ID_CARRITO = @CARRITO ORDER BY ID_OBJETO ASC LIMIT 1);

        INSERT INTO VENTAS_DETALLE
            VALUES(
            @TICKET,
            @OBJ,
            (SELECT CARR_CANT FROM CARRITO_COMPRAS_DETALLE WHERE ID_CARRITO = @CARRITO ORDER BY ID_OBJETO ASC LIMIT 1),
            _COMMENTARIO,
            _CALIF,
            (SELECT OBJ_VEND FROM OBJETOS WHERE ID_OBJ = @OBJ),
            (SELECT ID_OBJETO FROM CARRITO_COMPRAS_DETALLE WHERE ID_CARRITO = @CARRITO ORDER BY ID_OBJETO ASC LIMIT 1)
        );

        DELETE FROM CARRITO_COMPRAS_DETALLE WHERE ID_CARRITO = @CARRITO AND ID_OBJETO = @OBJ;
